...
Updates and removal are managed from Unraid's **Plugins** page. The plugin has no separate web page or configuration step for normal use.
...
Optional settings are documented in [`src/settings.example.cfg`](src/settings.example.cfg). Copy it to `/boot/config/plugins/UGREEN-DXP4800GT-LED-Driver/settings.cfg` on the NAS and restart with `ugreen-gt-leds stop` followed by `ugreen-gt-leds start`.
